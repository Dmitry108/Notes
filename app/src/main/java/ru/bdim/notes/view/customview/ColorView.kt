package ru.bdim.notes.view.customview
...
import ru.bdim.notes.model.dip
...
class ColorView @JvmOverloads constructor(context: Context,
    ...
    init {
        val a = context.obtainStyledAttributes(attrs, R.styleable.ColorView)
        val defRadiusPx = dip(defRadiusDp).toFloat()
        radius = a.getDimension(R.styleable.ColorView_circleRadius, defRadiusPx)
        fillColorRes = a.getResourceId(R.styleable.ColorView_fillColor, R.color.white)
        val defStrokeWidthPx = dip(defStrokeWidthDp).toFloat()
        strokeWidth = a.getDimension(R.styleable.ColorView_strokeWidth, defStrokeWidthPx)
        strokeColorRes = a.getResourceId(R.styleable.ColorView_strokeColor, R.color.text_secondary)
        a.recycle()
    }
